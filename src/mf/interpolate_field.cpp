...
void MagneticField::interpolate_field() {
    auto new_x_grid = make_new_grid(this->x_grid_, this->scaling_coeffs_[0]);
    auto new_y_grid = make_new_grid(this->y_grid_, this->scaling_coeffs_[1]);
    auto new_z_grid = make_new_grid(this->z_grid_, this->scaling_coeffs_[2]);
    ...
    for (auto i = 0; i < this->x_grid_.size - 1; i ++) { // -1 because we process the last cell on the max_size - 1 step
        for (auto j = 0; j < this->y_grid_.size - 1; j ++) {
            for (auto k = 0; k < this->z_grid_.size - 1; k ++) {
    ...
    this->x_grid_ = new_x_grid;
    this->y_grid_ = new_y_grid;
    this->z_grid_ = new_z_grid;
...
void MagneticField::interpolate_in_cell(
    ...
    auto derivs1 = compute_derivatives(cell_i, cell_j, cell_k, this->x_grid_, this->y_grid_, this->z_grid_, this->field_);
    auto derivs2 = compute_derivatives(cell_i + 1, cell_j, cell_k, this->x_grid_, this->y_grid_, this->z_grid_, this->field_);
    auto derivs3 = compute_derivatives(cell_i, cell_j + 1, cell_k, this->x_grid_, this->y_grid_, this->z_grid_, this->field_);
    auto derivs4 = compute_derivatives(cell_i + 1, cell_j + 1, cell_k, this->x_grid_, this->y_grid_, this->z_grid_, this->field_);
    auto derivs5 = compute_derivatives(cell_i, cell_j, cell_k + 1, this->x_grid_, this->y_grid_, this->z_grid_, this->field_);
    auto derivs6 = compute_derivatives(cell_i + 1, cell_j, cell_k + 1, this->x_grid_, this->y_grid_, this->z_grid_, this->field_);
    auto derivs7 = compute_derivatives(cell_i, cell_j + 1, cell_k + 1, this->x_grid_, this->y_grid_, this->z_grid_, this->field_);
    auto derivs8 = compute_derivatives(cell_i + 1, cell_j + 1, cell_k + 1, this->x_grid_, this->y_grid_, this->z_grid_, this->field_);
    ...
    for (auto node_i = 0; node_i <= this->scaling_coeffs_[0]; node_i++) {
        auto x_tail = double(node_i) / this->scaling_coeffs_[0]; // works only for regular grid
        for (auto node_j = 0; node_j <= this->scaling_coeffs_[1]; node_j++) {
            auto y_tail = double(node_j) / this->scaling_coeffs_[1];
            for (auto node_k = 0; node_k <= this->scaling_coeffs_[2]; node_k++) {
                auto z_tail = double(node_k) / this->scaling_coeffs_[2];
                ...
                new_field[
                    node_i + cell_i * this->scaling_coeffs_[0]
                ][
                    node_j + cell_j * this->scaling_coeffs_[1]
                ][
                    node_k + cell_k * this->scaling_coeffs_[2]
                ] = f;
            }
        }
    }
    ...
    for (auto i = cell_i; i <= cell_i + 1; i++) {
        for (auto j = cell_j; j <= cell_j + 1; j++) {
            for (auto k = cell_k; k <= cell_k + 1; k++) {
                new_field[
                    i * this->scaling_coeffs_[0]
                ][
                    j * this->scaling_coeffs_[1]
                ][
                    k * this->scaling_coeffs_[2]
                ] = this->field_[i][j][k];
            }
        }
    }
